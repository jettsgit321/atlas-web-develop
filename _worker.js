...
if(D.faqs){var fl=gi('cms-faq-list');if(fl)fl.innerHTML=D.faqs.map(function(f){return'<div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)">'+f.q+'<span class="faq-icon">+<\/span><\/button><div class="faq-a"><p>'+f.a+'<\/p><\/div><\/div>';}).join('');}
...
if(D.trustBar){var tb=gi('cms-trust-bar');if(tb)tb.innerHTML=D.trustBar.map(function(item){return'<div class="trust-item"><div class="trust-icon">'+item.icon+'<\/div>'+item.text+'<\/div>';}).join('');}
