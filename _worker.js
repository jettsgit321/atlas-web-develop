...
if(D.faqs){var fl=gi('cms-faq-list');function hesc(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}if(fl)fl.innerHTML=D.faqs.map(function(f){return'<div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)">'+hesc(f.q)+'<span class="faq-icon">+<\/span><\/button><div class="faq-a"><p>'+hesc(f.a)+'<\/p><\/div><\/div>';}).join('');}
