...
<div class="field" style="display:flex;align-items:center;gap:14px;background:#f9fafb;border:1px solid #e5e7eb;padding:14px 16px;border-radius:10px;margin-bottom:18px">
  <div id="ann-toggle" onclick="annToggle()" style="width:48px;height:26px;border-radius:13px;background:\${a.enabled?'#16a34a':'#d1d5db'};position:relative;cursor:pointer;transition:background .2s;flex-shrink:0">
    <div style="width:20px;height:20px;border-radius:50%;background:#fff;position:absolute;top:3px;left:\${a.enabled?'25px':'3px'};transition:left .2s;box-shadow:0 1px 4px rgba(0,0,0,.2)"></div>
  </div>
  <span id="ann-status" style="font-size:14px;font-weight:600;color:#374151">\${a.enabled?'Banner is ON — showing on your site':'Banner is OFF'}</span>
</div>
<div class="field"><label class="field-label">Banner text</label><input class="f-input" type="text" id="ann-text" value="\${e(a.text)}" placeholder="e.g. 🔥 First 5 clients get 3 months free — only 2 spots left!" oninput="C.announcement.text=this.value;annUpdate()"/></div>
<div class="field"><label class="field-label">Background color</label><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:6px">\${['#16a34a','#0ea5e9','#f59e0b','#dc2626','#7c3aed','#111827'].map(col=>\`<div onclick="C.announcement.bg='\${col}';annUpdate();document.querySelectorAll('.cswatch').forEach(s=>s.style.boxShadow='');this.style.boxShadow='0 0 0 3px #fff,0 0 0 5px '+'\${col}'" class="cswatch" style="width:34px;height:34px;border-radius:9px;background:\${col};cursor:pointer;transition:transform .15s;\${a.bg===col?'box-shadow:0 0 0 3px #fff,0 0 0 5px '+col:''}" onmouseover="this.style.transform='scale(1.12)'" onmouseout="this.style.transform='scale(1)'"></div>\`).join('')}</div></div>
</div>
<div style="margin-top:4px"><div style="font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px">Live preview</div><div id="ann-preview-bar" style="background:\${a.bg||'#16a34a'};color:#fff;text-align:center;padding:12px 20px;font-size:14px;font-weight:600;border-radius:10px;transition:background .2s;min-height:44px;display:flex;align-items:center;justify-content:center">\${e(a.text)||'<span style="opacity:.5">Your announcement text will appear here</span>'}</div></div>
<script>
function annToggle(){C.announcement.enabled=!C.announcement.enabled;const on=C.announcement.enabled;document.getElementById('ann-toggle').style.background=on?'#16a34a':'#d1d5db';document.getElementById('ann-toggle').querySelector('div').style.left=on?'25px':'3px';document.getElementById('ann-status').textContent=on?'Banner is ON — showing on your site':'Banner is OFF';}
function annUpdate(){const t=C.announcement.text||'';const b=C.announcement.bg||'#16a34a';const bar=document.getElementById('ann-preview-bar');if(bar){bar.style.background=b;bar.innerHTML=t||'<span style="opacity:.5">Your announcement text will appear here</span>';}}
<\/script>\`}
